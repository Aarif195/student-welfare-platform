...
export const forgotPasswordController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const ownerCheck = await pool.query("SELECT id FROM HostelOwners WHERE email = $1", [email]);
    const studentCheck = await pool.query("SELECT id FROM students WHERE email = $1", [email]);
